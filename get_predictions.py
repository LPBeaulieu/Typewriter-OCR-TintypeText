...
def get_character_x_y_coordinates(image):
    ...
    for char_y_min_max in chars_y_min_maxes:
        ...
        (line_index > 0 and char_y_min_max[0]-chars_y_min_maxes[line_index-1][0] < 2*character_width)):
            ...
            x_pixels = np.where(line_image >= 5)[0]
...
learn = load_learner(cwd + '/typewriter_OCR_cnn_model_bs64_ep3_lr005_acc9996_new')
...
with alive_bar(len(image_names)) as bar:
    with open(path + OCR_text_file_name + '-OCR.rtf', 'a+') as f:
        ...
        for image_name in image_names:
            ...
            #x-axis minimum of a character and the x-axis maximum of the preceding character exceeds 20%
            ...
            for i in range(len(text)-1, -1, -1):
                ...
                #If the label is a "space", it is replaced with " ".
                    ...
                    text[i] = " "
                #If the label is "forward slash", it is replaced with a backslash,
                #as a backslash is more useful in rtf files than a forward slash.
            ...
            for i in range(0, len(text)):
                #Replace "=" with a backslash if the following list element has a length of 1
