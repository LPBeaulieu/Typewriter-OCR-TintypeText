...
def get_character_x_y_coordinates(image):
    ...
    for char_y_min_max in chars_y_min_maxes:
        ...
        (line_index > 0 and char_y_min_max[0]-chars_y_min_maxes[line_index-1][0] < 2*character_width)):
            ...
            #of ink, you should increase the value of 3 to 5 in the line 56 in order to avoid including
            #unwanted noise in the character rectangles.
            ...
            for char_x_min_max in sorted(chars_x_min_maxes, reverse=True):
                ...
                #both my 2021 Royal Epoch and 1968 Olivetti Underwood Lettra 33 typewriters). Also, if your
                #typewriter has a lot of ghosting (faint outline of the preceding character), the segmentation
                #code might pick up the ghosting as characters, and you would end up with many overlapping
                #characters. If such were the case, you would need to make the segmentation less
                #inclusive/sensitive by increasing the decimal multiplyer from 0.40*character_width to about
                #0.55*character_width. Conversely, if your typewriter has very little ghosting, you would want
                #to make the segmentation more inclusive/sensitive by decreasing the decimal multiplyer
                #from about 0.55*character_width to 0.40*character_width, in order to avoid missing out
                #on some information. <b>These parameters ("character_width", "spacer_between_characters"
                #and "0.40*character_width" or "0.55*character_width" ) should be adjusted in the same way
                #in all the Python code files (except "train_model.py", where they are absent),
                #to ensure consistent segmentation in all steps of the process.
